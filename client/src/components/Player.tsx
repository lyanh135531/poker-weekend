...
const Player: React.FC<PlayerProps & { turnExpiresAt?: number }> = ({
  ...
  return (
    <>
      ...
      >
        <div className="flex -space-x-3 sm:-space-x-5 relative">
                ...
                return (
                  ...
                  >
                    <Card
                      ...
                      hidden={
                        isMasked ||
                        (!isMe && (stage !== "SHOWDOWN" || !!isFoldVictory))
                      }
                    />
                  </motion.div>
                );
            ...
            {isMasked && !player.isFolded && player.cards.length > 0 && (
              ...
              >
                <div className="glass-ui-gold px-3 py-1 rounded-sm border-poker-gold/40 shadow-2xl backdrop-blur-md">
                  <span className="text-[10px] font-black text-poker-gold uppercase tracking-[0.3em] italic drop-shadow-md">
                    PRIVATE
                  </span>
                </div>
              </motion.div>
          ...
          >
            ...
            <div className="relative w-5 h-8 mr-2 overflow-visible">
              <AnimatePresence mode="popLayout">
                {[
                  ...Array(
                    Math.min(Math.floor(player.bet / (bigBlind || 20)) + 1, 8),
                  ),
                ].map((_, i) => (
                  <motion.div
                    key={i}
            ...
            <div className="glass-ui-gold px-4 py-1.5 rounded-full border-poker-gold/30 shadow-[0_15px_35px_rgba(0,0,0,0.8),0_0_20px_rgba(195,163,91,0.2)] border flex items-baseline gap-1 backdrop-blur-2xl">
              <span className="text-poker-gold/60 text-[7px] font-black uppercase tracking-widest">
                $
              </span>
      ...
      >
        ...
        >
          <div
            className={`relative p-1 rounded-full transition-all duration-500 scale-110 ${player.isTurn ? "turn-glow-circular" : ""}`}
          >
            ...
            <div
              className={`w-[var(--player-plate-width)] h-[var(--player-plate-width)] rounded-full border border-white/10 overflow-hidden shadow-2xl relative bg-slate-950 transition-all duration-300 ${isMe ? "ring-2 ring-poker-gold/50 ring-offset-2 ring-offset-slate-950" : ""} ${player.isTurn ? "ring-2 ring-poker-gold" : ""}`}
            >
              <div
                className={`w-full h-full rounded-full flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 to-black outline-none transition-all duration-300 ${player.isTurn ? "opacity-100" : "opacity-80"}`}
              >
                ...
                <div className="flex items-center justify-center z-10">
                  <span className="text-poker-gold/50 text-[5px] md:text-[7px] mr-0.5 font-bold leading-none">
                    $
                  </span>
